  ...
  if (lang === 'JavaScript' || lang === 'TypeScript') {
    ...
    try {
      ...
      return Object.keys({ ...(pkg.dependencies ?? {}), ...(pkg.devDependencies ?? {}) })
        .map((d) => d.replace(/^@/, '').split('/')[0]) // normalize scoped packages
...
/**
 * Fetch package manifests for all repos and return GraphLinks for repos sharing ≥2 significant deps.
 * Runs in batches to respect rate limits. Call after initial graph is displayed.
 */
